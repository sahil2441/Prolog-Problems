...
hw3(InputListofLists, Output):-
    flatten1(InputListofLists,FlatList),
    sudoku(FlatList,Output),
    printoutput(Output,35).
...
test(N,Result) :-    puzzle(N,P), hw3(P,Result).
...
puzzle(3,P):-
    P=
    [ [_, _, 4, 5, _, _],[_, 6, _, _, 2, _],[5, _, _, _, _, 2],[1, _, _, _, _, 4],
    [_, 3, _, _, 4, _],[_, _, 2, 3, _, _] ].
